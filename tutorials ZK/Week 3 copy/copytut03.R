...
poissonfunc <- function(param, input, outcome) {
    beta <- (param[1] + param[2] * input)
    lambda <- exp(beta)
    -sum(dpois(outcome, lambda, log=TRUE))
}
results_p <- optim(fn=poissonfunc, outcome=y_pois, input=X, par=c(0,1), hessian=T, method="BFGS")
results_p$par
